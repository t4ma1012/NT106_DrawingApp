...
function Write-LoadBalancerSummary {
    param(
        ...
        $displayHost = if ([string]::IsNullOrWhiteSpace($PublicHost)) { '<ngrok-host>' } else { $PublicHost }

    Write-Host "--- $ModeLabel summary ---"
        Write-Host ("Public endpoint: {0}:{1}" -f $displayHost, $PublicPort)
    ...
    Write-Host ("Server 2 backend: {0}:{1}/{2}" -f $Server2Host, $Server2TcpPort, $Server2UdpPort)
        Write-Host ("Client command: scenario-3-internet.ps1 -Role Client -LbHost {0} -LbPort {1}" -f $displayHost, $PublicPort)
}
...
switch ($Role) {
    'LoadBalancer' {
        if ($LbPort -le 0) { $LbPort = Ask-Int 'Nhap port public cua LoadBalancer' 9000 }
        if ([string]::IsNullOrWhiteSpace($Server1Host)) { $Server1Host = Ask-String 'Nhap IP/host server 1 (Tailscale/LAN)' '127.0.0.1' }
        ...
        if ([string]::IsNullOrWhiteSpace($Server2Host)) { $Server2Host = Ask-String 'Nhap IP/host server 2 (Tailscale/LAN)' '127.0.0.1' }
        ...
        Write-LoadBalancerSummary -ModeLabel 'Internet/Tailscale' -PublicHost $LbHost -PublicPort $LbPort -Server1Host $Server1Host -Server1TcpPort $Server1TcpPort -Server1UdpPort $Server1UdpPort -Server2Host $Server2Host -Server2TcpPort $Server2TcpPort -Server2UdpPort $Server2UdpPort

        if ($StartNgrok.IsPresent) {
            Start-NgrokWindow -Port $LbPort
        }
    }
    'Server1' {
        if ([string]::IsNullOrWhiteSpace($Server1Host)) { $Server1Host = Ask-String 'Nhap IP/host server 1 (Tailscale/LAN)' '127.0.0.1' }
        ...
        Invoke-RoleWindow `
            -Title 'DrawingServer-1-Internet' `
    ...
    'Server2' {
        ...
        Invoke-RoleWindow `
            -Title 'DrawingServer-2-Internet' `
            -WorkingDir (Join-Path $RepoRoot 'DrawingServer') `
            -EnvLines @(
    ...
    'Client' {
        if ([string]::IsNullOrWhiteSpace($LbHost)) { $LbHost = Ask-String 'Nhap host cua ngrok/LoadBalancer' }
        if ($LbPort -le 0) { $LbPort = Ask-Int 'Nhap port cua ngrok/LoadBalancer' 9000 }
...
Write-Host "Internet/Tailscale scenario role started: $Role"
if ($StartNgrok.IsPresent) {
    Write-Host 'Read the ngrok Forwarding line, then pass that host/port to the three client roles.'
}
